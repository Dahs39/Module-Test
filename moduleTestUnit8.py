...
filepath = 'c:\\Python\\Script_Pictures\\moduleTest\\Screenshots\\Unit8\\%s' %currentDay
if not os.path.exists(filepath):
	os.makedirs('c:\\Python\\Script_Pictures\\moduleTest\\Screenshots\\Unit8\\%s' %currentDay)
...
def Screenshot_with_Paint(name):
	...
	sleep(15)
	pyautogui.typewrite('c:\\Python\\Script_Pictures\\moduleTest\\Screenshots\\Unit8\\')
	sleep(1)
...
# Selects module from moduleDataUnit8 and loads it, takes a screenshot after preset sleep time
def injectModule(moduleName):
	selectAllModules(moduleName)
	print('Currently loading: {}'.format(moduleName))
	print(time.strftime('%m-%d-%Y-%H:%M:%S'))
	sleep(moduleDataUnit8[moduleName][0])
	btnPos.click_ContinueBtn(moduleDataUnit8[moduleName][1])
	if moduleDataUnit8[moduleName][2] == True:
		btnPos.click_CallTutorBtn()
	Screenshot_with_Paint(moduleName)

Local_Frontend_Login('dchau_test', 'Cangetin00')

#Iterates through modules in moduleDataUnit8dictionary
for module in moduleDataUnit8:
	injectModule(module)
